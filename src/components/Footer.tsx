    ...
    <footer className="relative overflow-hidden bg-mist border-t border-border text-navy-deep">
      ...
      <div className="relative mx-auto max-w-7xl px-6 pt-24 pb-10">
        <div className="grid lg:grid-cols-12 gap-12 pb-16 border-b border-border">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between pt-8 text-xs text-muted-foreground/60">
            <span>
              {global.footer?.copyright ||
                `© ${new Date().getFullYear()} GURMITRAA. All rights reserved.`}
            </span>

            {global.footer?.madeByText && global.footer?.madeByUrl ? (
              <a
                href={global.footer.madeByUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-3 text-xs text-muted-foreground hover:text-orange transition"
                title={global.footer.madeByText}
              >
                {global.footer?.madeByLogoUrl ? (
                  <img
                    src={global.footer.madeByLogoUrl}
                    alt={global.footer.madeByText}
                    className="h-6 w-auto object-contain rounded-sm"
                  />
                ) : null}
                <span className="font-semibold">{global.footer.madeByText}</span>
              </a>
            ) : null}
          </div>
      </div>
