...
import { updateAdminPasswordInDb, updateAdminEmailInDb } from "@/lib/admin-auth";
...
export function AdminDashboard({ adminEmail, onLogout }: { adminEmail: string, onLogout: () => void }) {
  ...
  const [mediaItems, setMediaItems] = useState<any[]>(() => getLocalMediaCache());
  ...
  const handleMediaUploaded = (asset: MediaAsset) => {
    setMediaItems((prev) => {
      const updated = [asset, ...prev.filter((m) => m.id !== asset.id)];
  ...
  // Load global CMS data and set up real-time sync
  useEffect(() => {
    ...
    // Load global CMS data from Cloudinary on mount
    ...
    // Load submissions from Firebase (Firebase RTDB may still work for sessions/submissions on some projects)
    // Use a one-time get that times out gracefully
    const submissionsController = new AbortController();
    fetch(`https://gurumitraa-default-rtdb.firebaseio.com/submissions.json`, {
      ...
      // Submissions not available — that's OK
    });

    // Media: start from local cache (which includes default assets)
    setMediaItems(getLocalMediaCache());
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              onClick={async () => {
                if (
                  window.confirm(
                    "⚠️ RESET ALL CMS: Are you sure you want to reset ALL pages and global settings back to the initial factory templates? Any custom content will be cleared. (Cloudinary media files will NOT be deleted)."
                  )
                ) {
                  try {
                    await resetAllCms();
                    toast.success("All CMS configurations reset to factory defaults in Firebase RTDB!");
                    setTimeout(() => window.location.reload(), 300);
                  } catch (err: any) {
                    toast.error(err.message || "Failed to reset CMS configurations.");
                  }
                }
              }}
              className="bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 hover:text-red-300 px-3.5 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
              title="Reset everything in CMS to factory defaults"
            >
              <RotateCcw size={14} />
              <span>Reset All CMS</span>
            </button>
...
function ChangePasswordManager() {
  ...
  const handleEmailChange = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateAdminEmailInDb(newEmail.trim());
      if (res.success) {
        setEmailSent(true);
        toast.success("Administrator email updated successfully!");
      } else {
...
function GlobalConfigManager({
  ...
  const [globalState, setGlobalState] = useState<any>(() => {
    const draft = getDraft("global");
    return draft ? deepMerge(DEFAULT_GLOBAL, draft) : (initialGlobalData || DEFAULT_GLOBAL);
  });
  const [hasUnsaved, setHasUnsaved] = useState(() => !!getDraft("global"));
  const [saving, setSaving] = useState(false);

  const updateGlobalField = (path: string[], value: any) => {
    setGlobalState((prev: any) => {
      const updated = JSON.parse(JSON.stringify(prev || DEFAULT_GLOBAL));
      let current = updated;
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await publishGlobalCms(globalState);
      clearDraft("global");
      setHasUnsaved(false);
      toast.success("✅ Global settings published and live on the site!");
    } catch (err: any) {
      console.error("[CMS] Global publish error:", err);
      toast.error(`❌ Publish failed: ${err.message || "Unknown error"}. Your draft is preserved.`);
      // DO NOT set hasUnsaved=false — draft stays so admin can retry
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {hasUnsaved && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-semibold">You have unsaved working draft changes in Global Settings.</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => {
                clearDraft("global");
                setGlobalState(initialGlobalData || DEFAULT_GLOBAL);
                setHasUnsaved(false);
                toast.info("Draft discarded, reverted to published version.");
              }}
              className="px-4 py-1.5 text-xs font-bold hover:bg-white/5 border border-white/10 rounded-md transition cursor-pointer"
            >
              Discard Draft
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-orange hover:bg-orange/95 px-4 py-1.5 text-xs font-bold rounded-md transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="flex items-center justify-between pt-4 sticky bottom-0 bg-[#070b13] py-4 border-t border-white/10">
        <button
          type="button"
          onClick={async () => {
            if (window.confirm("Are you sure you want to reset Global Settings back to default in Firebase RTDB?")) {
              try {
                const def = await resetGlobalCms();
                setGlobalState(def);
                setHasUnsaved(false);
                toast.success("Global Settings reset to factory defaults in Firebase RTDB!");
              } catch (err: any) {
                toast.error(err.message || "Failed to reset global settings");
              }
            }
          }}
          className="bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 hover:text-white px-5 py-3 rounded-full text-xs font-bold transition flex items-center gap-2 cursor-pointer"
        >
          <RotateCcw size={14} />
          <span>Reset Global Defaults</span>
        </button>
        ...
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="bg-orange hover:bg-orange/90 text-white font-bold px-8 py-3.5 rounded-full shadow-lg hover:scale-[1.02] disabled:opacity-50 disabled:scale-100 transition duration-300 flex items-center gap-2 cursor-pointer"
        >
          {saving ? <RefreshCw className="animate-spin" size={18} /> : <CheckCircle2 size={18} />}
          <span>{saving ? "Publishing..." : "Publish Global Configurations"}</span>
        </button>
      </div>
    </div>
  );
...
function PagesManager({
  ...
  const [pageState, setPageState] = useState<any>(() => JSON.parse(JSON.stringify(PAGE_CONFIGS.home.defaults)));
  const [initialDbData, setInitialDbData] = useState<any>(() => JSON.parse(JSON.stringify(PAGE_CONFIGS.home.defaults)));
  ...
  const saveNewPageOrder = async (newPageKeys: string[]) => {
    try {
      ...
      const updatedGlobal = { ...globalData, navbar: newNavbar };
      await publishGlobalCms(updatedGlobal);
      setGlobalData?.(updatedGlobal);
      toast.success("Navigation menu reordered and saved successfully!");
    } catch (err: any) {
      console.error("Failed to save page order:", err);
      toast.error(err.message || "Failed to save page order");
    }
  };
  ...
  const togglePageVisibility = async (key: string) => {
    try {
      ...
      const updatedGlobal = { ...globalData, navbar: updatedNavbar };
      await publishGlobalCms(updatedGlobal);
      setGlobalData?.(updatedGlobal);
      toast.success("Page visibility updated and saved successfully!");
    } catch (err: any) {
      console.error("Failed to toggle page visibility:", err);
      toast.error(err.message || "Failed to update page visibility");
    }
  };

  // Load page from CMS (Cloudinary) and restore draft if present
  useEffect(() => {
    const config = PAGE_CONFIGS[selectedPage];
    const draft = getDraft(`page_${selectedPage}`);

    if (draft) {
      const mergedDraft = deepMerge(config.defaults, draft);
      setPageState(mergedDraft);
      setHasUnsaved(true);
    } else {
      setPageState(JSON.parse(JSON.stringify(config.defaults)));
      setHasUnsaved(false);
    }
    setEditingSectionIndex(null);

    // Fetch the currently published version from Cloudinary
    fetchPageData(selectedPage).then((data) => {
      if (data) {
        // Sort sections by order
        if (data.sections) {
          data.sections.sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0));
        }
        setInitialDbData(JSON.parse(JSON.stringify(data)));
        // Only set page state if no draft exists (don't overwrite admin's active edits)
        if (!getDraft(`page_${selectedPage}`)) {
          setPageState(data);
          setHasUnsaved(false);
        }
      }
    }).catch((err) => {
      console.warn(`[CMS] fetchPageData(${selectedPage}) notice:`, err);
    });

    // Subscribe to live changes from Cloudinary (polls every 30s)
    const unsub = subscribeToPageChanges(selectedPage, (data) => {
      if (data) {
        if (data.sections) {
          data.sections.sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0));
        }
        setInitialDbData(JSON.parse(JSON.stringify(data)));
        // Only update editor state if no unsaved draft
        if (!getDraft(`page_${selectedPage}`)) {
          setPageState(data);
          setHasUnsaved(false);
        }
  ...
  const updatePageState = (updater: (draft: any) => void) => {
    ...
    saveDraft(`page_${selectedPage}`, nextState);
    setHasUnsaved(true);
  };
  ...
  const handleSavePage = async () => {
    ...
    try {
      ...
      await publishPageCms(selectedPage, payload);
      clearDraft(`page_${selectedPage}`);
      setInitialDbData(JSON.parse(JSON.stringify(payload)));
      setPageState(payload);
      setHasUnsaved(false);
      toast.success(`✅ ${PAGE_CONFIGS[selectedPage].label} published and live on the site!`);
    } catch (err: any) {
      console.error("[CMS] Page publish error:", err);
      toast.error(`❌ Publish failed: ${err.message || "Unknown error"}. Your draft is preserved.`);
      // DO NOT clear draft — admin can retry
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {hasUnsaved && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <AlertCircle className="text-yellow-500" size={20} />
            <p className="text-sm font-semibold">You have unsaved working draft changes on {PAGE_CONFIGS[selectedPage].label}.</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => {
                clearDraft(`page_${selectedPage}`);
                setPageState(initialDbData || JSON.parse(JSON.stringify(PAGE_CONFIGS[selectedPage].defaults)));
                setHasUnsaved(false);
                toast.info("Draft discarded, reverted to published version.");
              }}
              className="px-4 py-1.5 text-xs font-bold hover:bg-white/5 border border-white/10 rounded-md transition cursor-pointer"
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={async () => {
                  if (
                    window.confirm(
                      `Are you sure you want to reset ${PAGE_CONFIGS[selectedPage].label} sections back to factory default in Firebase RTDB?`
                    )
                  ) {
                    try {
                      const def = await resetPageCms(selectedPage);
                      setPageState(def);
                      setInitialDbData(JSON.parse(JSON.stringify(def)));
                      setHasUnsaved(false);
                      setEditingSectionIndex(null);
                      toast.success(`${PAGE_CONFIGS[selectedPage].label} reset to factory defaults in Firebase RTDB!`);
                    } catch (err: any) {
                      toast.error(err.message || "Failed to reset page");
                    }
                  }
                }}
                className="bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 hover:text-white px-3 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
                title="Reset this page layout back to defaults"
              >
                <RotateCcw size={13} />
              ...
              <div className="relative group">
                <button className="bg-white/5 border border-white/10 hover:bg-white/10 px-3 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1.5 cursor-pointer">
                  <Plus size={14} />
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={handleSavePage}
                    disabled={saving}
                    className="bg-orange hover:bg-orange/90 text-white font-bold px-4 py-1.5 text-xs rounded-md shadow-md hover:scale-[1.02] disabled:opacity-50 transition flex items-center gap-1.5 cursor-pointer"
                  >
                    {saving ? <RefreshCw className="animate-spin" size={13} /> : <CheckCircle2 size={13} />}
      ...
      {/* Floating Sticky Quick-Publish Bar */}
      {hasUnsaved && (
        <div className="fixed bottom-6 right-8 z-40 bg-[#0d1424]/95 border-2 border-orange/80 rounded-2xl shadow-2xl p-3 px-5 flex items-center gap-4 backdrop-blur-xl animate-bounce">
          <div className="flex items-center gap-2">
            <span className="h-2.5 w-2.5 rounded-full bg-orange animate-pulse" />
            <span className="text-xs font-bold text-white uppercase tracking-wider">Unsaved Changes Ready</span>
          </div>
          <button
            type="button"
            onClick={handleSavePage}
            disabled={saving}
            className="bg-orange hover:bg-orange/95 text-white font-bold px-5 py-2 rounded-xl shadow-lg shadow-orange/25 transition flex items-center gap-2 cursor-pointer text-xs uppercase tracking-wider"
          >
      ...
      <div className="flex justify-end pt-4 sticky bottom-0 bg-[#070b13] py-4 border-t border-t-white/10">
        ...
        >
          {saving ? <RefreshCw className="animate-spin" size={18} /> : <CheckCircle2 size={18} />}
          <span>{saving ? "Publishing..." : "Publish Page Layout"}</span>
        </button>
      </div>
    </div>
  );
...
function PremiumMediaFieldEditor({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onChange(asset.url);
      toast.success("File uploaded to Cloudinary and linked successfully!");
      if (fileInputRef.current) fileInputRef.current.value = "";
    } catch (err: any) {
      console.error("Upload error:", err);
      toast.error(`Upload Failed: ${err.message || "Failed to upload asset to Cloudinary"}`);
    } finally {
      setUploading(false);
      if (fileInputRef.current) fileInputRef.current.value = "";
    }
...
function MediaManager({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (isPickerMode) {
        onSelectMedia(asset.url);
      }
      toast.success("File uploaded to Cloudinary successfully!");
      if (fileInputRef.current) fileInputRef.current.value = "";
    } catch (err: any) {
      console.error("Upload error:", err);
      toast.error(`Upload Failed: ${err.message || "Could not upload asset to Cloudinary"}`);
    } finally {
      setUploading(false);
      if (fileInputRef.current) fileInputRef.current.value = "";
    }
  ...
  const deleteMedia = async (item: any) => {
    try {
      // 1. Check if asset is actively referenced anywhere across CMS
      const usage = await checkMediaUsage(item.url, item.publicId || item.id);

      if (usage.isUsed) {
        const confirmMsg = `⚠️ WARNING: This asset is currently in use on:\n• ${usage.occurrences.join(
          "\n• "
        )}\n\nDeleting it will permanently remove it from Cloudinary and will cause missing images on those pages.\n\nAre you sure you want to proceed?`;
        if (!window.confirm(confirmMsg)) return;
      } else {
        if (!window.confirm(`Are you sure you want to permanently delete "${item.name}" from Cloudinary?`))
          return;
      }

      await deleteMediaAsset(item);
      onMediaDeleted?.(item.id);
      toast.success("Media asset removed from Cloudinary and database");
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-md p-6 flex flex-wrap items-center justify-between gap-4">
        <div className="flex-1 min-w-[250px] relative">
          ...
          <input
            type="text"
            placeholder="Search Cloudinary media assets..."
            value={search}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={() => fileInputRef.current?.click()}
            disabled={uploading}
            className="bg-orange hover:bg-orange/90 text-white font-semibold px-5 py-2.5 rounded-md transition flex items-center gap-2 disabled:opacity-50 text-sm cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {filteredMedia.map((item) => {
          const isImage =
            !item.format ||
            ["jpg", "jpeg", "png", "webp", "gif", "svg", "raw"].includes(
              item.format?.toLowerCase(),
            ) ||
            item.url?.includes("res.cloudinary.com") ||
            item.url?.startsWith("data:image");

          const thumbnailUrl = isImage ? getThumbnailUrl(item.url, 400) : item.url;
                ...
                {isPickerMode && (
                  <div className="absolute inset-0 bg-[#000000a0] opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                    <button
                      ...
                      className="bg-orange text-white px-3 py-1.5 rounded-md text-xs font-bold shadow-lg hover:scale-105 transition cursor-pointer"
                    >
                      Use Asset
                    </button>
                  </div>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex gap-1.5 opacity-90 group-hover:opacity-100 transition">
                    <button
                      ...
                      className="p-1 hover:text-orange hover:bg-white/5 rounded transition cursor-pointer"
                    ...
                    {!isPickerMode && (
                      <button
                        ...
                        className="p-1 text-red-400 hover:text-red-300 hover:bg-red-500/5 rounded transition cursor-pointer"
        ...
        {filteredMedia.length === 0 && (
          <div
            onClick={() => fileInputRef.current?.click()}
            className="col-span-full text-center py-20 bg-white/5 border border-dashed border-white/15 hover:border-orange/50 hover:bg-white/10 rounded-md cursor-pointer transition"
          >
            <ImageIcon className="mx-auto text-white/20 mb-3" size={48} />
            <h4 className="font-semibold text-white/70">No Media Assets Found</h4>
            <p className="text-xs text-white/40 mt-1">
              Click anywhere in this box or use the &quot;Upload New Asset&quot; button above to select files.
            </p>
          </div>
        )}
      </div>
