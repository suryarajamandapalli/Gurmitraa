...
import { fetchPageData, deepMerge, subscribeToPageChanges } from "@/lib/cms";
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: "https://gurmitraa.vercel.app/contact" },
...
function Contact() {
  ...
  const [page, setPage] = useState<any>(pageData || DEFAULT_CONTACT);

  useEffect(() => {
    // Subscribe to live contact page updates
    const unsub = subscribeToPageChanges("contact", (data) => {
...
function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Honeypot spam check: if bot fills hidden website field, simulate success
    if (websiteHoneypot.trim().length > 0) {
      setSent(true);
      return;
    }
          ...
          >
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <input
                ...
                tabIndex={-1}
                autoComplete="off"
                className="opacity-0 absolute -left-[9999px] -top-[9999px] h-0 w-0 pointer-events-none"
              />
            </div>
